...
echo "$($PSQL "SELECT ROUND(AVG(winner_goals) + AVG(opponent_goals), 16) AS total_goals FROM games")"
...
echo "$($PSQL "SELECT name FROM teams FULL JOIN games ON (teams.team_id = games.opponent_id OR teams.team_id = games.winner_id) WHERE round='Eighth-Final' AND year=2014 ORDER BY name ASC")"
